  ...
  componentWillReceiveProps(newProps) {
    ...
    switch (newProps.state) {
      case ble.DEVICE_STATE_DISCONNECT:
        ...
        this.manager.cancelDeviceConnection(newProps.selectedDeviceId).then((successIdentifier) => {
      ...
      case ble.DEVICE_STATE_CONNECT:
        this.manager.connectToDevice(newProps.selectedDeviceId)
          .then((device) => {
            this.subscriptions[device.id] = device.onDisconnected((error, disconnectedDevice) => {
              newProps.pushError("Disconnected from " + (disconnectedDevice.name ? disconnectedDevice.name : disconnecteddevice.id))
              newProps.changeDeviceState(newProps.selectedDeviceId, ble.DEVICE_STATE_DISCONNECTED);
              this.subscriptions[device.id].remove()  
            })
    ...
    newProps.operations.forEach((value, key) => {
      ...
      switch (type) {
        case 'read':
          ...
          this.manager.readCharacteristicForDevice(deviceId, serviceId, characteristicId)
            .then((characteristic) => {
        ...
        case 'write':
          if (state !== 'new') return true
          this.manager.writeCharacteristicWithResponseForDevice(deviceId, serviceId, characteristicId, base64Value)
            .then((characteristic) => {
              newProps.completeTransaction(transactionId)
              newProps.updateCharacteristic(deviceId, serviceId, characteristicId, { value: characteristic.value })
            }, (rejected) => {
        ...
        case 'monitor':
          if (state === 'new') {
            ...
            this.manager.monitorCharacteristicForDevice(deviceId, serviceId, characteristicId, (error, characteristic) => {
              if (error) {
